...
CrossGem::CrossGem(float x, float y, ESprite sprite, ESprite bonus)
...
int CrossGem::activate(std::vector<std::vector<Gem*>>& gems, size_t i, size_t j, size_t width, size_t height) {
  ...
  if (xi >= 0 && xj >= 0 && xi < height && xj < width && gems.at(xi).at(xj)->getSprite() != ESprite::S_NONE)
    gems.at(xi).at(xj)->setSprite(m_bonus);

  if (xi1 >= 0 && xj1 >= 0 && xi1 < height && xj1 < width && gems.at(xi1).at(xj1)->getSprite() != ESprite::S_NONE)
  ...
  return static_cast<int>(EGemScore::GS_ÑROSS);
}